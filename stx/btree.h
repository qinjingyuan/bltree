...
public:
    // 返回范围内元素的数量
    inline static int range_ele_num(const node * n, int start, int end){
        int num = 0;
        for(; start < end; start++){
            if(n->bs[start] == true){
                num++;
    ...
    inline static uint16_t fun_last_ele_site(node* lf){
        int last_ele_site = lf->slotuse - 1;
        while(!lf->bs[last_ele_site]){
            last_ele_site--;
        }
        return static_cast<uint16_t>(last_ele_site);
    }
    ...
    struct slotsite{
        ...
        int nextseg(){
            return (site | (segsize - 1)) + 1; 
        }
        int prevseg(){
            return (site | (segsize - 1)) - segsize * 2  + 1; 
        }
        ...
        static bool issolid(int s, int solid){
            return (s & 7) < solid;
        }
        ...
        if(dq.size() != 0){
            ...
            for(int i = right_use; i < leafslotmax; i++){
                ...
                if(slotsite::issolid(i, solid)){
                    leaf_ele e = dq.front();
                    dq.pop();
                    newleaf->slotkey[i] = e.value;
                    newleaf->slotdata[i] = e.value;
                    newleaf->bs[i] = true;
                    // ++valid_count;
                }
            }
...
auto currentTime1 = std::chrono::high_resolution_clock::now();
        ...
        /* if this site element pointed by point value, remove lastest 3 bits. */
        if(! n->bs[point]){
            point = (point >> 3) << 3;
        }
...
// std::cout << "pre_target = " << pre_target << ",point = " << point << "\n";
int gap = abs(pre_target - point) ;
gaps[btree_level] += gap;
gaps_count[btree_level]++;
#endif
...
private:
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
        ...
        else // n->isleafnode() == true
        {
            ...
            if(next_gap == next_seg_site){
                ...
                std::copy_backward(leaf->slotkey + next_seg_site - 4, leaf->slotkey + next_seg_site + 1, leaf->slotkey + next_seg_site + 5);
                data_copy_backward(leaf->slotdata + next_seg_site - 4, leaf->slotdata + next_seg_site + 1, leaf->slotdata + next_seg_site + 5);
                bit_copy_backward(leaf->bs, next_seg_site - 4, next_seg_site + 1, next_seg_site + 5);
    ...
    void split_leaf_node_x(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        BTREE_ASSERT(leaf->isfull());
        ...
        newleaf->slotuse = leaf->slotuse - mid;
        newleaf->slotusevalid = range_ele_num(newleaf, 0, newleaf->slotuse);
...
public:
    ...
    void bulk_load_x(Iterator ibegin, Iterator iend)
    {
        ...
        for (size_t i = 0; i < num_parents; ++i)
        {
            ...
            for (unsigned short s = 0; s < n->slotuse; ++s)
            {
                n->slotkey[s] = leaf->slotkey[leaf->slotuse - 3];
                n->childid[s] = leaf;
                leaf = leaf->nextleaf;
            }
            n->childid[n->slotuse] = leaf;
            ...
            // track max key of any descendant.
            nextlevel[i].first = n;
            nextlevel[i].second = &leaf->slotkey[leaf->slotuse - 3];
...
public:
    ...
    bool erase_one_x(const key_type& key)
    {
        BTREE_PRINT("btree::erase_one(" << key << ") on btree size " << size());
        ...
        result_t result = erase_one_descend_x(key, m_root, NULL, NULL, NULL, NULL, NULL, 0);
...
private:
                               ...
                               inner_node* parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            // result_t myres = result_t(btree_not_found);
    ...
    result_t erase_one_descend_x(const key_type& key,
                               node* curr,
                               node* left, node* right,
                               inner_node* leftparent, inner_node* rightparent,
                               inner_node* parent, unsigned int parentslot)
    {
        ...
        if (curr->isleafnode())
        {
            ...
            // 获取最后一个元素的位置
            int last_ele_site = leaf->slotuse - 1;
            while(!leaf->bs[last_ele_site]){
                last_ele_site--;
            }

            if (slot == last_ele_site + 1)
            {
                if (parent && parentslot < parent->slotuse)
                {
                    BTREE_ASSERT(parent->childid[parentslot] == curr);
                    parent->slotkey[parentslot] = leaf->slotkey[last_ele_site];
                }
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        BTREE_PRINT("Scheduling lastkeyupdate: key " << leaf->slotkey[last_ele_site]);
                        myres |= result_t(btree_update_lastkey, leaf->slotkey[last_ele_site]);
                    }
                    else
                    {
                        BTREE_ASSERT(leaf == m_root);
    ...
    result_t merge_leaves_x(leaf_node* left, leaf_node* right, inner_node* parent)
    {
        ...
        int left_slotuse =  left->slotuse;
        int right_slotuse = right->slotuse;
        ...
        left->slotuse = left_slotuse + right_slotuse;
        assert(left_slotuse + right_slotuse >= 0);
    ...
    static result_t shift_left_leaf_x(leaf_node* left, leaf_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        int shiftnum = (((right_slotuse >> 3) - (left_slotuse >> 3)) >> 1) << 3 ;
        ...
        left->slotuse += shiftnum;
        int move_left_valid_num = range_ele_num(right, 0, shiftnum);
        ...
        // 获取最后一个元素的位置
        int last_ele_site = left->slotuse - 1;
        while(!left->bs[last_ele_site]){
            last_ele_site--;
        }

        // fixup parent
        if (parentslot < parent->slotuse) {
            parent->slotkey[parentslot] = left->slotkey[last_ele_site];
            return result_t(btree_ok);
        }
        else {  // the update is further up the tree
            return result_t(btree_update_lastkey, left->slotkey[last_ele_site]);
        }
    }
    ...
    static void shift_right_leaf_x(leaf_node* left, leaf_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        int move_left_valid_num = range_ele_num(left, left_slotuse - shiftnum, left_slotuse);
        right->slotuse += shiftnum;
        right->slotusevalid += move_left_valid_num;
